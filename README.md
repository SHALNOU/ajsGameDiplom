...
[![Build status](https://ci.appveyor.com/api/projects/status/1q628w7ltoqt08l7?svg=true)](https://ci.appveyor.com/project/AndreiZak90/js-advanced-diploma-az)
...
1. Вы написали авто-тест на функцию _calcTileType_
...
5. type - строка с одним из допустимых значений:
   _'swordsman'_, _'bowman'_, _'magician'_, _'daemon'_, _'undead'_, _'vampire'_.
...
_characterGenerator_ - функция генератор, которая формирует случайного персонажа
...
_Класс Team_ хранит команду игрока или соперника. Реализуйте хранение персонажей по вашему усмотрению.
...
_generateTeam_ - формирует команду на основе `characterGenerator`. Для формирования всех персонажей
...
3. Проверьте, выдаёт ли генератор `characterGenerator` бесконечно новые персонажи из списка (учёт аргумента _allowedTypes_)
4. Проверьте, в нужном ли количестве и диапазоне уровней (учёт аргумента _maxLevel_) создаются персонажи при вызове `generateTeam`
